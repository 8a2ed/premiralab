      ...
      <main id="top">
        ...
        <section className="hero">
          <div className="container hero-grid">
            <div>
              <div className="eyebrow">'3*H/JH *5EJE 1BEJ E*C'ED</div>
              <h1>F-HQD 'D#AC'1 %DI <span className="highlight">*,'1( (51J)</span> BHJ).</h1>
              <p>EF 'DGHJ) 'D(51J) %DI 'DEF*,'* 'D1BEJ)  EF8HE) *5EJE '-*1'AJ) E9 %/'1) E4'1J9 HEDA'* HE1',9'* EF8E).</p>
              <div className="actions">
                <button className="btn btn--primary btn--lg" onClick={() => openOrder()}>
                  '7D( E41H9C <ArrowLeft size={18} aria-hidden />
                </button>
              </div>
            </div>
        ...
        <section className="section" id="services" aria-labelledby="services-title">
          <div className="container">
            <h2 id="services-title">'D./E'*</h2>
            <p className="muted">-DHD *5EJE B'(D) DD*H39.</p>
        ...
        <section className="section" id="packages" aria-labelledby="packages-title">
          <div className="container">
            <h2 id="packages-title">'D('B'*</h2>
            <p className="muted">'.*1 FB7) 'D(/'J) 'DEF'3().</p>
            <div className="grid grid-3" style={{ marginTop: 28 }}>
              {data.packages?.map(p => (
                <div className={`card package-card ${p.popular ? 'package-card--popular' : ''}`} key={p.id}>
                  {p.popular && <span className="tag">'D#C+1 7D('K P</span>}
                  <h3>{p.title}</h3>
                  <div className="price">{money(p.price, data.site?.currency)}</div>
                  <p className="muted">{p.description}</p>
                  <ul className="feature-list" aria-label="EEJ2'* 'D('B)">
                    {p.features.map(f => (
        ...
        <section className="section" id="portfolio" aria-labelledby="portfolio-title">
          <div className="container">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 10 }}>
              <div>
                <h2 id="portfolio-title">#9E'D E.*'1)</h2>
                <p className="muted">FE'0, H*,'1( (51J) 5EEF'G' D9ED'&F' ('6:7 9DI #J 9ED D'3*91'6 'D*A'5JD 'DC'ED)).</p>
              </div>
            </div>
            ...
            <div className="grid grid-3" style={{ marginTop: 28 }}>
              {data.portfolio?.length ? data.portfolio.map(p => (
                ...
                <div className="empty">#6A #9E'DC EF DH-) 'D%/'1).</div>
              )}
            </div>
          </div>
        </section>
        ...
        <section className="section" id="testimonials" aria-labelledby="testimonials-title">
          <div className="container">
            <h2 id="testimonials-title">"1'! 'D9ED'!</h2>
            <div className="grid grid-3" style={{ marginTop: 28 }}>
              {data.testimonials?.map(t => (
                <div className="card testimonial-card" key={t.id}>
                  <div className="stars" aria-label={`*BJJE ${t.rating} EF 5`}>
...
function OrderModal({ packages, services, defaultPackage, initialProjectType, onClose, onDone }: OrderModalProps) {
  ...
  if (submitted) {
    ...
    return (
      <Modal title="*E '3*D'E 7D(C (F,'- (" onClose={onClose}>
        <div className="order-success" style={{ textAlign: 'center', padding: '20px 0' }}>
          ...
          <h3 style={{ fontSize: 24, marginBottom: 8 }}>4C1'K D+B*C (F'!</h3>
          <p style={{ fontSize: 16 }}>1BE 7D(C: <strong style={{ color: 'var(--primary)', fontSize: 18, background: 'var(--primary-dim)', padding: '4px 10px', borderRadius: 8 }}>{submitted.orderNo}</strong></p>
          <p className="muted" style={{ maxWidth: 400, margin: '16px auto' }}>*E -A8 7D(C H3J*E E1',9*G H'D*H'5D E9C B1J('K. JECFC E*'(9) -'D) 'D7D( AJ #J HB*.</p>
          
          <div style={{ background: 'var(--bg-2)', padding: 20, borderRadius: 16, border: '1px solid var(--border)', marginTop: 24 }}>
            <p className="muted" style={{ margin: '0 0 12px', fontSize: 13 }}>1'(7 'DE*'(9) 'D.'5 (C ('-*A8 (G):</p>
            ...
            <button className="btn btn--sm" onClick={() => copyTrackerUrl(trackerUrl)} style={{ width: '100%', justifyContent: 'center' }} type="button">
              {copied ? <><Check size={16} className="icon--success" /> *E 'DF3.</> : <><Copy size={16} /> F3. 1'(7 'DE*'(9)</>}
            </button>
          </div>
          <button className="btn btn--primary" onClick={onClose} style={{ marginTop: 20, width: '100%' }}>%:D'B</button>
        </div>
      </Modal>
    );
  }
  ...
  const nextStep = () => {
    ...
    if (step === 1) {
      if (!f.packageId && !f.serviceId && !f.projectType) {
        setError('J1,I '.*J'1 ('B) #H ./E) #H C*'() FH9 'DE41H9 'DE.55 DDE*'(9)');
        return;
      }
    }
    ...
    if (step === 3) {
      if (!f.name || !f.phone) {
        setError(''D'3E H1BE 'DG'*A E7DH('F DD*H'5D');
        return;
      }
      ...
      if (!phoneRegex.test(f.phone.trim())) {
        setError('J1,I %/.'D 1BE G'*A 5-J- ECHF EF 11 1BE (E+'D: 01012345678)');
        return;
      }
      ...
      if (f.email) {
        ...
        if (!emailRegex.test(f.email.trim())) {
          setError('J1,I %/.'D (1J/ %DC*1HFJ 5-J-');
          return;
        }
      }
    }
  ...
  return (
    <Modal title="'(/# E41H9C 'D"F" onClose={onClose} size="lg">
      ...
      <div style={{ minHeight: 280, position: 'relative' }}>
        ...
        {step === 1 && (
          <div className="animation-fade-in">
            <h3 style={{ marginBottom: 20, fontSize: 18 }}>E' GH FH9 'D./E) 'D*J *(-+ 9FG'</h3>
            <div className="grid grid-2" style={{ gap: 20 }}>
              <div className="form-field">
                <label className="form-label" htmlFor="order-pkg">'D('B'* 'D,'G2)</label>
                <select id="order-pkg" className="select" style={{ padding: 14, fontSize: 15, borderColor: error && !f.packageId && !f.serviceId && !f.projectType ? '#ef4444' : undefined }} value={f.packageId} onChange={e => updateF({ packageId: e.target.value, serviceId: e.target.value ? '' : f.serviceId })}>
                  <option value="">'.*1 ('B) ('.*J'1J)</option>
                  {packages.map(p => <option key={p.id} value={p.id}>{p.title}  {money(p.price)}</option>)}
                </select>
              </div>
              <div className="form-field">
                <label className="form-label" htmlFor="order-svc">'D./E'* 'DA1/J)</label>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, margin: '24px 0' }}>
              ...
              <div className="muted" style={{ fontSize: 12 }}>#H *.5J5 E41H9C</div>
              <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
            </div>

            <div className="form-field">
              <label className="form-label" htmlFor="order-type">FH9 'DE41H9 'DE.55</label>
              <input id="order-type" className="input" style={{ padding: 14, borderColor: error && !f.packageId && !f.serviceId && !f.projectType ? '#ef4444' : undefined }} placeholder="E+'D: GHJ) (51J) C'ED) + *5EJE EHB9..." value={f.projectType} onChange={e => updateF({ projectType: e.target.value })} />
            </div>
          </div>
        )}
        ...
        {step === 2 && (
          <div className="animation-fade-in">
            <h3 style={{ marginBottom: 20, fontSize: 18 }}>E*7D('* H*A'5JD %6'AJ)</h3>
            <div className="form-grid">
              <div className="form-field">
                <label className="form-label" htmlFor="order-budget">'DEJ2'FJ) 'DE*HB9) ('.*J'1J)</label>
                <div style={{ position: 'relative' }}>
                  ...
                  <span style={{ position: 'absolute', right: 14, top: 14, color: 'var(--muted)', fontSize: 14 }}>,.E</span>
                </div>
              </div>
              <div className="form-field">
                <label className="form-label" htmlFor="order-deadline">'DEH9/ 'DFG'&J DD*3DJE</label>
            ...
            <div className="form-field" style={{ marginTop: 20 }}>
              <label className="form-label" htmlFor="order-notes">F(0) 9F 'DE41H9 H#G/'AG</label>
              <textarea id="order-notes" className="textarea" rows={4} maxLength={2000} style={{ padding: 14 }}
                placeholder="5A DF' AC1*C E*7D('*C 'D.'5) #H #J 1H'(7 E1,9J)..."
        ...
        {step === 3 && (
          <div className="animation-fade-in">
            <h3 style={{ marginBottom: 20, fontSize: 18 }}>CJA JECFF' 'D*H'5D E9C</h3>
            <div className="form-field" style={{ marginBottom: 16 }}>
              <label className="form-label" htmlFor="order-name">'D'3E 'DC'ED *</label>
              <input id="order-name" className="input" style={{ padding: 14, borderColor: error && !f.name ? '#ef4444' : undefined }} required placeholder="E-E/ #-E/" value={f.name} onChange={e => updateF({ name: e.target.value })} />
            </div>
            <div className="form-grid">
              <div className="form-field">
                <label className="form-label" htmlFor="order-phone">1BE 'DG'*A / 'DH'*3'( *</label>
              ...
              <div className="form-field">
                <label className="form-label" htmlFor="order-email">'D(1J/ 'D%DC*1HFJ ('.*J'1J)</label>
        ...
        {step === 4 && (
          <div className="animation-fade-in">
            <h3 style={{ marginBottom: 20, fontSize: 18, textAlign: 'center' }}>E1',9) 'D7D(</h3>
            <div style={{ background: 'var(--bg-2)', borderRadius: 16, border: '1px solid var(--border)', padding: 20, marginBottom: 20 }}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px 24px' }}>
                <div><span className="muted" style={{ fontSize: 12, display: 'block' }}>'D'3E</span><strong>{f.name}</strong></div>
                <div><span className="muted" style={{ fontSize: 12, display: 'block' }}>1BE 'D*H'5D</span><strong style={{ direction: 'ltr', display: 'inline-block' }}>{f.phone}</strong></div>
                
                {f.packageId && <div><span className="muted" style={{ fontSize: 12, display: 'block' }}>'D('B) 'DE.*'1)</span><strong style={{ color: 'var(--primary)' }}>{packages.find(p => p.id === Number(f.packageId))?.title}</strong></div>}
                {f.serviceId && <div><span className="muted" style={{ fontSize: 12, display: 'block' }}>'D./E) 'DE.*'1)</span><strong style={{ color: 'var(--primary)' }}>{services.find(s => s.id === Number(f.serviceId))?.title}</strong></div>}
                {f.projectType && <div><span className="muted" style={{ fontSize: 12, display: 'block' }}>FH9 'DE41H9</span><strong>{f.projectType}</strong></div>}
                
                {f.budget && <div><span className="muted" style={{ fontSize: 12, display: 'block' }}>'DEJ2'FJ) 'DEB*1-)</span><strong>{money(Number(f.budget))}</strong></div>}
                {f.deadline && <div><span className="muted" style={{ fontSize: 12, display: 'block' }}>'DEH9/ 'DFG'&J</span><strong>{f.deadline}</strong></div>}
              </div>
              {f.notes && (
                <div style={{ marginTop: 16, paddingTop: 16, borderTop: '1px solid var(--border)' }}>
                  <span className="muted" style={{ fontSize: 12, display: 'block' }}>ED'-8'*</span>
              ...
              <div style={{ marginTop: 24, padding: '16px', background: 'rgba(205, 69, 205, 0.05)', borderRadius: 12, border: '1px dashed var(--primary-dim)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
                  ...
                  <strong style={{ fontSize: 14 }}>71B 'D/A9 'DE*'-)</strong>
                </div>
                <p className="muted" style={{ fontSize: 13, margin: 0, lineHeight: 1.6 }}>
                  (9/ *#CJ/ 'D7D( 3J*E %5/'1 A'*H1) 1BEJ) JECFC /A9G' (3GHD) 9(1:
                  <br />
                  <span style={{ color: 'var(--text)', fontWeight: 500 }}>InstaPay AH/'AHF C'4 #H 'D*-HJD 'D(FCJ</span>.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="stepper-footer-actions" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 24, paddingTop: 20, borderTop: '1px solid var(--border)', gap: 16 }}>
        {step > 1 ? (
          <button type="button" className="btn" onClick={() => setStep(s => s - 1)} disabled={loading}>'D3'(B</button>
        ) : (
          <button type="button" className="btn" onClick={onClose} style={{ color: 'var(--muted)', background: 'transparent', borderColor: 'transparent' }}>%D:'!</button>
        )}
        
        {step < 4 ? (
          <button type="button" className="btn btn--primary" onClick={nextStep} style={{ minWidth: 120 }}>'D*'DJ</button>
        ) : (
...
function CaseStudyModal({ item, onClose, onOrder, whatsapp, brand }: CaseStudyModalProps) {
  const waUrl = whatsapp
    ? waLink(whatsapp, `E1-('K ${brand || 'PREMIRALAB'} #9,(FJ E41H9 "${item.title}" H#1J/ *FAJ0 E41H9 E4'(G.`)
    : null;
      ...
      <div className="case-study-modal">
        ...
        <div style={{ marginTop: 18, display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          <div style={{ background: 'var(--bg-3)', padding: 18, borderRadius: 'var(--radius-sm)', lineHeight: 1.8 }}>
            <h4 style={{ margin: '0 0 8px', fontSize: 15, color: 'var(--text)' }}>9F G0' 'D9ED:</h4>
            <p style={{ margin: 0, color: 'var(--text-muted)' }}>
              {item.description || 'E41H9 *5EJE HGHJ) (51J) 1BEJ) E*C'ED) *E *7HJ1G HAB #9DI E9'JJ1 'D,H/) H'D*,1() 'D(51J).'}
            </p>
          </div>
        </div>
      </div>
...
function FloatingWhatsApp({ whatsapp, brand }: { whatsapp?: string; brand?: string }) {
  ...
  const quickLinks = [
    { title: '7D( 916 391 31J9', msg: `E1-('K ${brand || 'PREMIRALAB'} #1J/ 'D-5HD 9DI 916 391 DE41H9J 'D,/J/.` },
    { title: ''3*A3'1 9F 'D('B'* 'DE*'-)', msg: `E1-('K ${brand || 'PREMIRALAB'} D/J '3*A3'1 (.5H5 ('B'* 'D*5EJE.` },
    { title: 'E*'(9) 7D( B'&E', msg: `E1-('K ${brand || 'PREMIRALAB'} #1J/ 'D'3*A3'1 9F -'D) 7D(J.` },
  ];
          ...
          <div className="floating-wa-header">
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 11, color: '#25D366' }}>E*5D 'D"F  F1/ .D'D /B'&B</div>
              </div>
            </div>
            <button className="btn btn--icon btn--sm" onClick={() => setOpen(false)} aria-label="%:D'B">
              <X size={16} />
            </button>
          </div>

          <p style={{ fontSize: 13, margin: '12px 0 10px', color: 'var(--text-muted)' }}>
            E1-('K (C! CJA JECFF' E3'9/*C 'DJHE '.*1 EF 'D.J'1'* 'D31J9):
          </p>
